...
type Sample = { roomId: string; round: number; time?: string; contentVersion?: string; trainingBots: number; players: Player[] };
...
const valid = (row: Sample) => typeof row.roomId === 'string' && nonnegative(row.round) && nonnegative(row.trainingBots)
  ...
  && row.players.every(player => player && typeof player === 'object' && nonnegative(player.level) && player.level >= 1 && player.level <= 5 && nonnegative(player.xp)
    ...
    && player.choices.every(choice => choice && typeof choice === 'object' && typeof choice.id === 'string' && Object.hasOwn(GROWTH_UPGRADES, choice.id) && nonnegative(choice.latency)));
...
export function growthPlaytestReport(lines: string[]) {
  ...
  for (const line of lines) {
    ...
    const key = JSON.stringify([value.contentVersion, value.roomId, value.round, value.time, value.players]);
  ...
  return { version: 1, acceptedResults: rows.length, malformed, duplicates,
