...
默认 ws://127.0.0.1:4180，仅提供联机后端，不包含或托管游戏前端。默认仅本机可连接。Ctrl+C 停止，活动房间不会保存。
...
配套客户端使用本地 Windows 便携 ZIP；解压后双击 PLAY.cmd 进入联机大厅。
`);
writeFileSync(join(folder, 'manifest.json'), JSON.stringify({ builtAt: new Date().toISOString(), contentVersion: packageContentVersion(), node: '>=22.12',
  entry: 'server.cjs', webRoot: false, frontend: 'none', sha256: createHash('sha256').update(readFileSync(join(folder, 'server.cjs'))).digest('hex'),
