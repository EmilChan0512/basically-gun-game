...
export function startCampaign() {
  ...
  function show(next: Screen) {
    ...
    if (screen === 'menu') {
      ...
      $('campaign-overlay').innerHTML = `<div class="compact-panel result-panel"><p class="eyebrow">${won ? 'MISSION COMPLETE' : 'MISSION FAILED'}</p><h2>${won ? '行动成功' : '行动未完成'}</h2><p class="result-score">${b.scores[0]} <span>—</span> ${b.scores[1]}</p><p class="story">${won ? b.mission.debrief : b.reason + '。利用掩体和队友的火力，或在任务地图降低难度再试。'}</p><p>个人击杀 ${b.player.kills} · 阵亡 ${b.player.life.deaths} · 用时 ${Math.ceil(b.frame / 30)} 秒${won ? ' · ' + '★'.repeat(progress.data.best[b.mission.id]) : ''}</p><div class="panel-actions">${won ? `<button id="next-mission" class="primary">${selected === MISSIONS.length - 1 ? '查看结局' : '下一关'}</button>` : ''}<button id="result-retry" ${won ? '' : 'class="primary"'}>重试本关</button><button id="result-home">任务地图</button></div></div>`;
      if (won) bind('next-mission', () => selected === MISSIONS.length - 1 ? show('ending') : brief(selected + 1));
  ...
  function brief(index: number) {
    ...
    selected = index;
    scene?.loadBattle(new Battle(MISSIONS[index], progress.data.difficulty, progress.loadout.primary, Math.random, progress.loadout));
    show('briefing');
  }
  function launch() {
    if (!scene) return;
    reward = null; scene.audio.unlock(); scene.loadBattle(new Battle(MISSIONS[selected], progress.data.difficulty, progress.loadout.primary, Math.random, progress.loadout));
    $('mission-label').textContent = MISSIONS[selected].title;
    $('goal-label').textContent = `目标 ${MISSIONS[selected].goal}`;
    show('playing');
  }
  ...
  window.addEventListener('strike-campaign-ready', ((event: CustomEvent<CampaignScene>) => {
    ...
    scene.onFrame = () => {
      ...
      $('player-ammo').textContent = `${p.arsenal.selected.toUpperCase()}  ${gun.ammo} / ${gun.reserveAmmo}${gun.reloadFrames ? ` · 换弹 ${(gun.reloadFrames / 30).toFixed(1)}s` : gun.ammo === 0 ? ' · Q切枪 / 返回出生区补给' : ''}`;
