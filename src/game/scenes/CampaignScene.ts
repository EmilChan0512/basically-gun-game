...
export class CampaignScene extends Phaser.Scene {
  ...
  create() {
    ...
    const down = (event: KeyboardEvent) => {
      ...
      if (!event.repeat && event.code === 'KeyQ') this.battle.swap();
      if (!event.repeat && (event.code === 'KeyR' || event.code === 'KeyL')) this.battle.reload();
      if (!event.repeat && event.code === 'KeyE') this.battle.useSkill();
      if (!event.repeat && event.code === 'KeyG') this.battle.useItem(this.input.activePointer.positionToCamera(this.cameras.main) as Phaser.Math.Vector2);
    };
  ...
  clearInput() { this.keys.clear(); this.mouse = false; this.jumpQueued = false; this.battle.releaseInput(); }
  loadBattle(battle: Battle) { this.clearInput(); this.battle = battle; this.lastFrame = 0; this.drawMap(battle.mission); }
  private drawMap(mission: Mission) {
    if (!this.background) return;
    this.cameras.main.setBounds(0, 0, mission.width, 700);
  ...
  private soldier(actor: Actor) {
    ...
    this.rig.soldier(x, y, m.crouching, m.vx, m.jumping, this.battle.frame, actor.aim, actor.arsenal.selected, color, actor.life.alive,
      actor.arsenal.gun.reloadFrames > 0, this.battle.effects.some(e => Math.abs(e.trace.origin.x - x) < 1 && Math.abs(e.trace.origin.y - (y - (m.crouching ? 28 : 42))) < 1 && this.battle.frame - e.frame < 2));
  ...
  update(_time: number, delta: number) {
    ...
    if (this.activeBattle) {
      ...
      const beforeFrame = this.battle.frame;
      this.battle.advance(Math.min(delta, 100), { ...idleInput(), left: has('KeyA', 'ArrowLeft'), right: has('KeyD', 'ArrowRight'),
        crouch: has('KeyS', 'ArrowDown'), jump: this.jumpQueued || has('KeyW', 'ArrowUp', 'Space'), fire: has('KeyF') || this.mouse, aim: pointer });
      if (this.battle.frame !== beforeFrame) this.jumpQueued = false;
    }
    const b = this.battle; this.cameras.main.centerOn(b.player.movement.x, 355);
    this.rig.begin();
    ...
    if (b.frame > this.lastFrame) {
      const newShots = b.effects.filter(effect => effect.frame > this.lastFrame);
      if (newShots.length) this.audio.cue('shot');
      if (newShots.some(effect => effect.damage && effect.team === 1)) this.audio.cue('hit');
      this.lastFrame = b.frame;
    }
