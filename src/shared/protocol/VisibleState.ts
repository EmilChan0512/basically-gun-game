...
export function visibleState(message: StateMessage, visible: ReadonlySet<string>, team: 1 | 2, wall: (x: number, y: number) => boolean): StateMessage {
  ...
  return { ...message,
    ...
    events: message.events.filter(e => (e.targetId === message.actorId && ['damage', 'death'].includes(e.kind)) || (!e.position || pointVisible(e.position)) && (e.kind.startsWith('objective-') || e.kind === 'result'
