...
import { DEFAULT_GROWTH_V3_PERKS, validateGrowthPerks, type GrowthPerkId } from './Perks';
...
export function defaultGrowthLoadoutV3(classId: GrowthClassId = 'assault'): GrowthLoadoutV3 {
  ...
  return { classId, abilityId, gadgetId: operator.gadgets[0], primary: operator.primary, secondary: 'usp',
    attachments: { primary: [], secondary: [] }, perks: [...DEFAULT_GROWTH_V3_PERKS], pool: defaultGrowthCards(classId, abilityId), title: 'none' };
}
/** Strict boundary: migration may fill missing fields; public save requests must never do so. */
export function validateGrowthLoadoutV3(value: unknown, stage: ContentStage): GrowthLoadoutV3 {
  ...
  const perks = validateGrowthPerks(value.perks), allowed = legalGrowthCards(classId, abilityId);
...
export function changeGrowthAbility(loadout: GrowthLoadoutV3, abilityId: GrowthAbilityId): GrowthLoadoutV3 {
  ...
  return { ...structuredClone(loadout), abilityId, pool };
}
