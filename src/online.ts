...
function abilityText(actor: { growthV3?: { abilityId: GrowthAbilityId; gadgetId: GrowthGadgetId }; growth?: { classId: GrowthClassId }; classId?: string | null; stealthFrames?: number; skill?: SkillId | null; skillCooldown: number; skillFrames: number; item?: ItemId | null; itemCharges: number; itemCooldown?: number; life?: { alive: boolean } }, charge?: { armed: boolean }) {
  if (actor.growthV3) return 'E ' + GROWTH_V3_ABILITIES[actor.growthV3.abilityId].name + ' · ' + (actor.skillFrames ? '生效中' : actor.skillCooldown ? Math.ceil(actor.skillCooldown / 30) + '秒' : '就绪') + ' | G ' + GROWTH_V3_GADGETS[actor.growthV3.gadgetId].name + ' ×' + actor.itemCharges + (charge ? actor.life?.alive === false ? ' · 已部署，复活后可引爆' : charge.armed ? ' · 按 G 引爆' : ' · 武装中' : actor.itemCharges === 0 ? ` · ${Math.ceil((actor.itemCooldown ?? 0) / 30)}秒后恢复` : '');
...
export class OnlineScene extends Phaser.Scene {
  ...
  create() {
    ...
    else if (this.network.state!.mapId === 'longshot') drawSpaceStation(this,map);
    else {
