...
import { GROWTH_V3_GADGETS as GROWTH_GADGETS, resolveGadget } from '../../shared/content/growth-v3/Gadgets';
...
export class GrowthMatchView {
  ...
  requestFailed() { this.pendingBatch = ''; this.error = '选择未生效，请重试'; this.choiceKey = ''; this.render(this.message); }
  render(message: StateMessage | null) {
    ...
    if (batch !== this.batch) {
      const reroll = !!this.pendingBatch && !this.pendingUpgrade && !!g.offer;
      this.batch = batch; this.pendingBatch = ''; this.pendingUpgrade = ''; this.deferred = !reroll; this.error = '';
    }
    ...
    if (overviewKey !== this.overviewKey) {
      ...
      overview.innerHTML = `<div class="growth-command-strip"><img class="growth-avatar" src="${operatorConcept(g.classId)}" alt=""><div class="growth-rank"><small>局内成长</small><h2>${GROWTH_CLASSES[g.classId].name} · Lv.${g.level}</h2></div><div class="growth-xp"><span>${g.xp} XP${g.level < 5 ? ` / ${upper}` : ' · 普通等级已满'}</span><div class="growth-xp-track" role="progressbar" aria-label="当前等级进度" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${Math.round(progress)}"><i style="width:${progress}%"></i></div></div><div class="growth-picks"><small>已选升级</small><strong>${g.selected.length}<span> / 4</span></strong></div><div class="growth-awakening${g.ultimate ? ' is-awakened' : ''}"><small>${g.ultimate ? '已觉醒' : `第${g.ultimateTick / 1800}分钟觉醒${g.preset === 'short' ? ' · 短局实验' : ''}`}</small><strong>${growthIcon(GROWTH_CLASSES[g.classId].ultimate)}${ult.name.split(' · ').at(-1)}</strong></div></div><div class="growth-build-line"><details${expanded ? ' open' : ''}><summary>技能与构筑详情</summary><div class="growth-build-details"><p>当前构筑</p>${g.selected.length ? g.selected.map(id => `<span class="growth-build-chip">${GROWTH_UPGRADES[id].name.split(' · ').at(-1)}</span>`).join('') : '<span class="growth-build-empty">尚未选择局内升级</span>'}<p>E ${GROWTH_ABILITIES[g.abilityId].name} · ${growthIcon(g.gadgetId)} G ${GROWTH_GADGETS[g.gadgetId].name}（可用${g.gadget.charges}次，用后${resolveGadget(g.gadgetId, g.gadget.upgraded, g.gadget.empowered, g.gadget.extraGranted).cooldown / 30}秒恢复）。比赛不因选卡暂停。</p><p>职业特化：${g.perks.map(id => GROWTH_PERKS[id].name).join(' / ')} · 临时护甲 ${Math.ceil(g.armor)}</p><p>${ult.name}：${ult.description}</p></div></details></div>${g.classId === 'medic' ? `<p class="growth-healing">队友治疗 ${g.healingDone} · 治疗经验 ${g.healingXp}。自疗、环境伤害及重复刷取不计经验。</p>` : ''}`;
    }
    ...
    const title = document.createElement('div'); title.innerHTML = `<small>局内升级 · ${g.selected.length + 1} / 4</small><h3>选择你的下一步战术</h3><p>三选一 · 比赛继续进行 · 死亡期间也可选择${g.level - 1 - g.selected.length > 1 ? ` · 还有 ${g.level - 2 - g.selected.length} 次选择待处理` : ''}</p>`;
    const toggle = document.createElement('button'); toggle.dataset.growthToggle = ''; toggle.textContent = this.deferred ? `U · 可升级（${Math.max(1,g.level-1-g.selected.length)}）` : 'U · 稍后选择';
    toggle.setAttribute('aria-expanded', String(!this.deferred)); toggle.onclick = () => { this.deferred = !this.deferred; this.render(this.message); toggle.blur(); };
    ...
    const note = document.createElement('span'); note.textContent = pending ? '等待服务器确认…' : this.error || '升级保留至本局结束 · 下一局重新构筑'; note.setAttribute('role', 'status'); footer.append(note, reroll); selection.append(footer);
  }
}
