...
export class CombatFeedbackView {
  ...
  render(visible: boolean, observing?: string) {
    ...
    this.root.parentElement?.classList.toggle('death-grayscale', f.dead && !f.canObserve);
    ...
    setText(this.hint, f.noRevive ? '团队复活次数已用尽' : f.canObserve ? `正在观察 · ${observing ?? '阵亡位置 / 战场'}` : '即将观察队友');
  ...
  destroy() { this.root.parentElement?.classList.remove('death-grayscale'); this.root.remove(); }
}
