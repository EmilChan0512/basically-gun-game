...
export function mapPreviewSvg(map: MapDefinition, debug = false) {
  ...
  const art = g.artwork;
  const terrain = g.terrain.map(t => `<rect x="${t.x}" y="${t.y}" width="${t.width}" height="${t.height}" fill="${color(g.palette.wall)}"/>`).join('');
