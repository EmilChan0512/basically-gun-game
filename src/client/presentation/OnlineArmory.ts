...
export function renderOnlineArmory(root: HTMLElement, value: EquipmentLoadout, profile: OnlineProfile | null, debug: boolean,
  ...
  const cards = (slot: Slot) => {
    ...
    return catalog.sort((a, b) => a[1].level - b[1].level).map(([id, entry]) => {
      ...
      const metric = 'config' in entry ? `伤害 ${entry.config.damage}${entry.pellets > 1 ? ' × ' + entry.pellets : ''}　弹匣 ${entry.config.magazineSize}　射程 ${entry.config.rangeUnits * 10}`
        : 'cooldown' in entry ? entry.passive ? '被动技能 · 装备后自动触发 · 无需按 E' : `主动技能 · 冷却 ${entry.cooldown / 30} 秒${entry.duration ? `　持续 ${entry.duration / 30} 秒` : ''}`
  ...
  root.innerHTML = `<div class="visual-armory" id="online-loadout" style="--role-color:${color}">
    ...
    <div class="arsenal-workbench"><aside class="operator-roster" aria-label="选择职业"><p class="arsenal-section-label">01 / 作战职业</p>${classIds.map((id, index) => `<button id="class-${id}" data-class="${id}" aria-pressed="${id === classId}" class="operator-card ${id === classId ? 'is-active' : ''}"><span class="operator-number">0${index + 1}</span><img src="${operatorConcept(id)}" alt="${CLASSES[id].name}" draggable="false"><span><strong>${CLASSES[id].name}</strong><small>${debug ? '调试可用' : `Lv.${levelForXp(profile?.classes[id].xp ?? 0)}`}</small></span><span class="operator-selected" aria-hidden="true">${id === classId ? '●' : '○'}</span></button>`).join('')}</aside>
    <div class="operator-stage"><div class="operator-stage-heading"><span>OPERATOR / 0${classIds.indexOf(classId) + 1}</span><span>${debug ? 'LIVE TEST' : `LEVEL ${String(level).padStart(2, '0')}`}</span></div><div class="operator-pedestal"><img class="generated-operator" src="${operatorConcept(classId)}" alt="${definition.name} 作战概念像"><span class="stage-line" aria-hidden="true"></span></div><div class="operator-caption"><h2>${definition.name}</h2><p>${definition.passive}</p><div class="operator-stats"><span><small>生命</small><b>${definition.health}</b></span><span><small>瞄准</small><b>${Math.round(definition.aim * 100)}%</b></span><span><small>弹药</small><b>×${definition.ammo}</b></span></div></div></div>
    ...
    <div class="arsenal-inventory"><div class="arsenal-inventory-heading"><div><p class="arsenal-section-label">03 / 装备陈列</p><h2>${slots[ui.slot]}<small>${ui.slot === 'secondary' ? '刀仅限刺客 · 盾仅限重装兵' : ui.slot === 'skill' ? '主动按 E · 被动自动触发 · 每次装备一个' : ui.slot === 'item' ? 'G 使用 · 每次出战补充' : '职业专属主武器 · 按原作等级成长'}</small></h2></div><div class="arsenal-tabs" aria-label="装备分类">${Object.entries(slots).map(([slot, label]) => `<button id="tab-${slot}" data-slot-tab="${slot}" aria-pressed="${ui.slot === slot}">${label}</button>`).join('')}</div></div><div class="arsenal-grid" id="online-gear-grid">${cards(ui.slot)}</div></div>
