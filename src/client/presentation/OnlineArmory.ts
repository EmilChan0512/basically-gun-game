...
export function renderOnlineArmory(root: HTMLElement, value: EquipmentLoadout, profile: OnlineProfile | null, debug: boolean,
  ...
  const cards = (slot: Slot) => {
    const catalog = slot === 'primary' ? Object.entries(WEAPONS).filter(([, entry]) => entry.slot === 'primary')
    ...
    return catalog.map(([id, entry]) => {
      const owned = debug || slot === 'skill' || Object.hasOwn(SPECIAL_OFFHANDS, id) || (slot === 'item' ? profile?.items.includes(id as keyof typeof ITEMS) ?? id === 'medkit' : profile?.weapons.includes(id as WeaponId) ?? ['m4', 'usp'].includes(id));
  ...
  root.innerHTML = `<div class="visual-armory" id="online-loadout" style="--role-color:${color}">
    ...
    <div class="arsenal-workbench"><aside class="operator-roster" aria-label="选择职业"><p class="arsenal-section-label">01 / 作战职业</p>${classIds.map((id, index) => `<button id="class-${id}" data-class="${id}" aria-pressed="${id === classId}" class="operator-card ${id === classId ? 'is-active' : ''}"><span class="operator-number">0${index + 1}</span><img src="/assets/reference/${id}-head.png" alt="${CLASSES[id].name}" draggable="false"><span><strong>${CLASSES[id].name}</strong><small>${debug ? '调试可用' : `Lv.${levelForXp(profile?.classes[id].xp ?? 0)}`}</small></span><span class="operator-selected" aria-hidden="true">${id === classId ? '●' : '○'}</span></button>`).join('')}</aside>
  ...
  root.querySelectorAll<HTMLButtonElement>('[data-class]').forEach(button => button.onclick = () => {
    ...
    change(debug ? { ...equipment, classId: next, secondary: 'usp', skill: CLASSES[next].skills[0] } : profile?.classes[next].equipment ?? starterEquipment(next));
  });
