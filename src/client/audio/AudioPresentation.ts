...
export interface AudioActor { id: string; x: number; y: number; weapon: string; classId?: string | null; reload: number; life: { alive: boolean }; team: number }
...
export class AudioPresentation {
  ...
  accept(scope: string, tick: number, events: SimulationEvent[], actors: AudioActor[], listenerId?: string | null, winner?: number | null, audible = true) {
    if (scope !== this.scope) { this.audio.stop(); this.scope = scope; this.cooldown.clear(); this.reloading.clear(); this.classId = ''; }
    ...
    if (listener.id === listenerId && listener.classId && listener.classId !== this.classId && this.audio.voice(listener.classId)) this.classId = listener.classId;
    ...
    for (const event of fresh) {
      ...
      if (event.kind === 'shot' || event.kind === 'reload') {
        ...
        this.audio.cue('death', options); if (event.actorId === listenerId && event.targetId !== listenerId) this.audio.voice('kill');
