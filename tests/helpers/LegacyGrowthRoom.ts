// Retained v2 simulation harness only. Production Room and network tests use v3.
import { GROWTH_ACHIEVEMENTS } from '../../src/shared/content/GrowthRecords';
import { Battle, seededRandom } from '../../src/game/campaign/Battle';
import { GROWTH_RULES, defaultGrowthLoadout, validateGrowthLoadout, type GrowthLoadout } from '../../src/shared/content/GrowthCatalog';
...
import { validateEquipment, type EquipmentLoadout } from '../../src/shared/content/Equipment';
...
export class Room {
  ...
  round = 0;
  constructor(readonly id: string, public mapId = 'hijack', public mode: import('../../src/shared/simulation/ModeRules').ModeId = 'tdm', readonly debug = false, readonly rules: 'classic' | 'growth' = 'classic') {
  ...
  configure(id: string, mapId: string, mode: import('../../src/shared/simulation/ModeRules').ModeId) {
  ...
  start(id: string, seed: number) {
    ...
    const battle = new Battle({ ...map, ...(this.rules === 'growth' ? { seconds: GROWTH_RULES.seconds, goal: Number.MAX_SAFE_INTEGER } : {}), allies: soloGrowth ? 0 : blue - 1, enemies: soloGrowth ? 1 : red }, 'normal', 'm4', seededRandom(seed), null, `${this.id}:${++this.round}`);
    this.session = new MatchSession(battle);
    roster.forEach((player, i) => { player.spectator = false; battle.actors[i].name = player.name;
      ...
      if (this.rules === 'growth') battle.equipGrowth(battle.actors[i], player.growthLoadout); else battle.equipActor(battle.actors[i], player.equipment);
    ...
    if (soloGrowth) {
      ...
      bot.name = `${classId} 训练机器人`; battle.equipGrowth(bot, defaultGrowthLoadout(classId));
    }
  }
