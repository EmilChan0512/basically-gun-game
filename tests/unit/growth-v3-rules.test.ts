...
it('exhaustively preserves damage/fire rate/ammo bounds for legal three-slot builds on all 18 weapons', () => {
  let builds=0;
  for(const id of Object.keys(GROWTH_V3_WEAPONS) as GrowthWeaponId[]) {
    ...
    const visit=(chosen:GrowthAttachmentId[], start:number) => {
      const resolved=resolveGrowthWeapon(id,chosen); builds++;
  ...
  expect(builds).toBeGreaterThan(10000);
});
