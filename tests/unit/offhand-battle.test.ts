...
function fixture(kind: 'melee' | 'shield') {
  const battle = new Battle({ ...customMatch('signal'), allies: 0, enemies: 1 }, 'normal', 'm4', seededRandom(1));
...
it('runs knife damage through real Battle ticks, restores windup and suppresses guns', () => {
  ...
  expect(battle.actors[1].life.health).toBe(35);
  expect(battle.player.arsenal.shots).toBe(0);
  expect(battle.journal.since(0).filter(e => e.kind === 'damage')).toHaveLength(1);
});

it('uses deployed controller reduction and stows the shield on actual respawn', () => {
  ...
  expect(player.life.health).toBe(77.5); expect(player.offhand!.shield.durability).toBe(120);
...
it('forces the special offhand while carrying a case and restores selection on release', () => {
  const battle = new Battle({ ...customMatch('hijack'), mode: 'ctf', allies: 0, enemies: 1 }, 'normal', 'm4', seededRandom(1));
