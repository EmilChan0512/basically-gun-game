...
it.each([null, { type:'unit' as const, target:'enemy-0', region:'head' as const }])('keeps a distant shot visible without exposing hidden hit data: %j', hit => {
  ...
  const trace={origin:{x:100,y:458},end:{x:3100,y:458},maxDistance:3300,steps:300,preSteps:0,initialHit:hit,hit,headMarked:!!hit};
  ...
  expect(result.effects[0].trace.end.x).toBeGreaterThan(2400);
  expect(result.effects[0].trace.end.x).toBeLessThan(2500);
  expect(result.effects[0]).toMatchObject({damage:0,killed:false,trace:{hit:null,initialHit:null,headMarked:false}});
  expect(message.effects[0].trace.end.x).toBe(3100);
});
