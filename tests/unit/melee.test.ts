...
it('honors windup, hits once per target and prevents restarting during recovery', () => {
  ...
  for (let i = 0; i < 3; i++) expect(swing.tick('self', 1, origin, targets, () => false)).toEqual([]);
...
it('filters friends, dead, out-of-range and rear targets and respects thin walls', () => {
  ...
  for (let i = 0; i < 3; i++) swing.tick('self', 1, origin, targets, () => false);
...
it('restores one-hit-per-swing and does not cleave a newly entering target', () => {
  const swing = new MeleeSwing(); swing.start(origin, aim);
  ...
  expect(restored.checkpoint().hitIds).toEqual(['first']);
});
