...
import { CLASS_STARTERS, type ClassId, CLASSES, SKILLS, ITEMS } from '../../src/game/campaign/Catalog';
...
it('changes only the controlled debug actor in place and clears old actions, cooldowns and projectiles', () => {
  ...
  room.equip('a', { classId: 'tank', primary: 'shotgun', secondary: 'siegius', skill: 'iron', item: 'frag' });
  ...
  expect(actor.offhand?.id).toBe('siegius'); expect(actor.arsenal.selected).toBe('shotgun');
...
it('validates every class skill and tactical item, rejects forged configurations atomically', () => {
  ...
  for (const [classId, definition] of Object.entries(CLASSES)) for (const skill of definition.skills) {
    for (const item of Object.keys(ITEMS)) expect(validateEquipment({ classId, skill, item, primary: CLASS_STARTERS[classId as ClassId], secondary: 'usp' })).toMatchObject({ classId, skill, item });
  }
  ...
  for (const equipment of [
    ...
    { classId: 'assassin', primary: 'scout', secondary: 'knife', skill: 'constructor' },
...
it('retains live debug choices across death, reconnect and restoration', () => {
  ...
  room.equip('a', { classId: 'tank', primary: 'shotgun', secondary: 'blast-shield', skill: 'barrier', item: 'frag' });
...
it('keeps ordinary match equipment immutable, even through the debug-only API', () => {
  ...
  room.equip('a', { classId: 'assassin', primary: 'scout', secondary: 'knife', skill: 'cloak' });
