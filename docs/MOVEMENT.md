...
默认沙盒采用 OriginalMovement 的30Hz离散移动、蹲伏、跳跃、空中操控、攀爬与实心地形采样；旧120Hz Arcade实验只在 `?rules=lab` 使用。自研沙盒边界为x=0..2400，脚部y超过840触发死亡并进入复活，不再沿用参考地图2880边界。手感仍待用户试玩评价，逐帧原版一致性不作为当前交付门槛。

## Phase 3 第四批边界（2026-09-10）

本批未修改移动参数、碰撞体或120Hz物理时钟，也未新增原版移动观测。新增的站立26×66/蹲姿26×44矩形是原版**子弹命中框**，以Unit脚底坐标计算，不能据此替换移动碰撞体。场景从士兵身体中心发射仍为枪口坐标的TUNED替代；命中框以1:1实验像素接入，实际姿态、枪口和运动坐标映射待后续核对。现有移动值继续保持TUNED，原版移动校准仍未验收。

2026-09-10 Phase 3第二批：移动与碰撞仍按120Hz运行，本轮未修改移动默认值。射击改为每4个物理步推进1个30Hz战斗步，并在该步先尝试射击再递减计数。暂停/慢速沿用同一仿真时间；单步只推进1/120秒。此调整不代表Movement、攀爬、平台或角色姿态已与原版对齐，连续运动容差仍待原版测量后预先锁定。
